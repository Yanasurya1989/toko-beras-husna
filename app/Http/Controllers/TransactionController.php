...
class TransactionController extends Controller
{
    ...
    public function store(Request $request)
    {
         ...
         *
         * Tidak titip  = amount_paid
         * Titip        = nominal_titip
         */
            if ($validated['titip'] === 'ya') {

                $amountPaid = $validated['nominal_titip'] ?? 0;
            } else {
            ...
            /*
         * Jangan sampai pembayaran yang tercatat
         * lebih besar dari total transaksi.
         */
            $paymentAmount = min(
                $amountPaid,
                $totalPrice
    ...
    public function createPayment(Transaction $transaction)
    {
        ...
        return view('transaksi.payment', compact('transaction'));
    }
    ...
    public function storePayment(Request $request, Transaction $transaction)
    {
         ...
         */
            $transaction->payments()->create([
                'payment_date' => now(),
                'amount' => $paymentAmount,
                'note' => $validated['note'] ?? 'Pembayaran hutang',
            ]);
         ...
         */
            $newAmountPaid =
                $transaction->amount_paid + $paymentAmount;
         ...
         */
            $newRemainingDebt =
                max(
                    $transaction->total_price - $newAmountPaid,
                    0
                );
     ...
     */
        ...
        $html .= '<td colspan="11"><strong>' .
            date('l, F d, Y', strtotime($tanggalAwal)) .
            '</strong></td>';
        $html .= '</tr>';
